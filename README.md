...
A small collection of scripts and notes for **Gaffer**.

## Contents
- [Variables editor guide](#variables-editor-guide)
  - [The key node: `NameSwitch`](#the-key-node-nameswitch)
  - [Step-by-step: Cloudy / Sunny light setups](#step-by-step-cloudy--sunny-light-setups)
...
### Step-by-step: Cloudy / Sunny light setups
...
3. **Label the `NameSwitch` inputs**
   - In the `NameSwitch` node editor, set the name for each input:
     - `in0 name = Cloudy`
     - `in1 name = Sunny`
...
5. **Connect the output to your LIGHTS group** (replacing the direct connections)
...
Now when you go `Variables` → `lightSetup` → `Sunny`, the `NameSwitch` instantly routes the **Sunny** branch through. Pick `Cloudy` and it switches.

---

### Other useful patterns

| What you want | Node to use | Selector / idea |
|---|---|---|
...
**Tip:** `NameSwitch` is especially artist-friendly—it matches strings directly, so your preset values only need to match the input names.